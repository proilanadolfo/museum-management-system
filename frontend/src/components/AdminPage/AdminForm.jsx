...
export default function AdminForm({ onClose }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (data?.token) {
        ...
        // Dispatch custom event to notify SuperAdminDashboard of admin login
        window.dispatchEvent(new CustomEvent('adminLoggedIn'))
        
        // Force page reload to show admin dashboard
  ...
  const handleGoogleLogin = () => {
    // Redirect directly to Google OAuth for admin
    window.location.href = '/api/google/admin'
  }
  ...
  const handleResetWithCode = async (e) => {
    ...
    try {
      const res = await fetch('/api/reset-password', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email: forgotEmail, code: resetCode, newPassword, userType: 'admin' })
      })
      if (!res.ok) {
        const body = await res.json().catch(() => ({}))
